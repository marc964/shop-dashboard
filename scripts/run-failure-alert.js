...
/**
 * True if the run died before any step executed — i.e. GitHub never handed us
 * a runner. That's their infrastructure, not our pipeline, and there is nothing
 * for a human to fix. A genuine failure has steps with real conclusions.
 */
async function isStartupCancel(runId) {
  try {
    const { jobs } = await ghGet(`/repos/${REPO}/actions/runs/${runId}/jobs`);
    if (!jobs || !jobs.length) return false;
    return jobs.every(
      (j) => !(j.steps || []).some((s) => s.conclusion === "success" || s.conclusion === "failure")
    );
  } catch {
    return false;
  }
...
async function main() {
  ...
  // Not wedged, so this is either GitHub failing to hand us a machine (nothing
  // to do) or our own pipeline breaking (check the log). Say which.
  const diagnosis = (await isStartupCancel(RUN_ID))
    ? `The jobs were *cancelled before any step ran* — GitHub failed to acquire a hosted runner. ` +
      ...
      `on our end. It clears on its own.\n• GitHub status: https://www.githubstatus.com`
    : `The job *did* run and failed partway through, so this one is likely ours — check the failing ` +
      `step in the run log.`;
